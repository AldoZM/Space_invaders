...
class NaveEsp2(pygame.sprite.Sprite):

    ####Clase de la nave####

    def __init__(self):
        pygame.sprite.Sprite.__init__(self)
        self.ImgenNave = pygame.image.load('defensor2.png')

        self.rect = self.ImgenNave.get_rect()
        self.rect.centerx = ancho/1.5
        self.rect.centery = alto-30
...
def SpaceInvader(): ####Funje como el main del programa####
    ...
    #miFuentePuntuaje = pygame.font.SysFont('Arial',30,True,True)
    #TextoPuntaje = miFuentePuntuaje.render("Tu puntaje: "+str(puntajeTiempo),0,(255,255,0))
    
    #fuenteContador = pygame.font.SysFont('Arial',30, True)
    #contador = fuenteContador.font.SysFont(""+str(puntajeTiempo),0, (255,255,0))
    ...
    while True:
        ...
        miFuentePuntuajeJ2 = pygame.font.SysFont('Arial',30,True,True)
        TextoPuntajeJ2 = miFuentePuntuajeJ2.render("Tu puntaje J2: "+str(puntosJ2),0,(255,255,0))
            ...
            if enJuego == True:
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == K_a:
                        ...
                        print("Dispara J1")
                #
                #   Aqui se establece los controles del J2
                #
                if event.type == pygame.KEYDOWN:
                    if event.key == K_LEFT:
                        jugador2.rect.left -= jugador.velocidad
                        print("LEFT J2")

                    elif event.key == K_RIGHT:
                        jugador2.rect.right += jugador.velocidad
                        print("RIGHT J2")
                    elif event.key == K_UP:
                        x,y = jugador2.rect.center
                        jugador2.dispara(x,y)
                        print("Dispara J2")
       ...
       #
        ...
        if len(listaEnemigo)>0:
            for enemigo in listaEnemigo:
                ...
                if enemigo.rect.colliderect(jugador2.rect):
                    detenerJuego()
                    jugador2.destruccion()
                        ...
                        if x.rect.top > 750:
                            ...
                            for disparo in jugador.listaDisparo:
                                if x.rect.colliderect(disparo.rect):
                                    ...
                                    enemigo.listaDisparo.remove(x)
                            for disparo in jugador2.listaDisparo:
                                if x.rect.colliderect(disparo.rect):
                                    jugador2.listaDisparo.remove(disparo)
                                    enemigo.listaDisparo.remove(x)
            #
        ...
        #
        if len(jugador2.listaDisparo)>0:
            for x in jugador2.listaDisparo:
                x.dibujar(ventana)
                x.trayectoria()

                if x.rect.top > 700:
                    jugador2.listaDisparo.remove(x)
                else:
                    for enemigo in listaEnemigo:
                        if x.rect.colliderect(enemigo.rect):
                            listaEnemigo.remove(enemigo)
        ...
        ventana.blit(TextoPuntajeJ2,(1050,0))
        pygame.display.update()
